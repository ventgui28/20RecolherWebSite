...
export default function PedidoRecolha() {
            ...
            <div key={step.id} className="flex flex-col items-center flex-1 relative">
              <div 
                className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-500 z-10 ${
                  currentStep >= step.id ? "bg-[#111111] text-white" : "bg-gray-50 text-gray-300"
                }`}
              >
                <step.icon className="h-5 w-5" />
              </div>
              <span className={`mt-3 text-[10px] font-bold uppercase tracking-widest ${
                currentStep >= step.id ? "text-[#111111]" : "text-gray-300"
              }`}>
                {step.name}
              </span>
              {step.id !== steps.length && (
                <div className={`absolute top-5 left-[calc(50%+20px)] w-[calc(100%-40px)] h-[1px] ${
                  currentStep > step.id ? "bg-[#111111]" : "bg-gray-100"
                }`} />
              )}
            </div>
          ))}
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mt-12 flex items-center justify-between pt-8 border-t border-gray-50">
              <button
                type="button"
                onClick={prevStep}
                className={`flex items-center space-x-2 text-[#111111] font-bold text-xs uppercase tracking-widest transition-opacity ${
